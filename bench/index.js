...
function benchSimple(cnt, cb) {
  ...
  function write(cb) {
    ...
    var start = +new Date;
    for (var i=0; i<cnt; i++) {
      var person = fsdb({ location: 'people' });
  ...
  function read(cb) {
    var start = +new Date;
    var people = fsdb('people').read(function(err) {
      if (err) throw err;
      var end = +new Date;
      var elapsed = ((end - start) / 1000).toFixed(3);
      console.log(people.folders.length + ' models read in ' + elapsed + ' seconds');
